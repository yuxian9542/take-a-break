...
if grep -q "GOOGLE_MAPS_API_KEY=" "$ENV_FILE"; then
  API_KEY=$(grep "GOOGLE_MAPS_API_KEY=" "$ENV_FILE" | cut -d '=' -f 2)
...
# 检查 USE_REAL_MAP_API
if grep -q "USE_REAL_MAP_API=" "$ENV_FILE"; then
  USE_REAL=$(grep "USE_REAL_MAP_API=" "$ENV_FILE" | cut -d '=' -f 2)
