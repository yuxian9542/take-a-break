<template>
  <VoiceShell
    ...
    @start="clickMedia(MEDIA_TYPE.AUDIO)"
    @stop="closeWS"
    @refreshLocation="fetchLocationContext"
  >
...
import VoiceShell from "@/components/VoiceShell.vue";
import {
  MEDIA_TYPE,
  MSG_TYPE,
  SOCKET_STATUS,
...
import { getNearbySpots, getUserPosition, buildSystemPrompt } from "@/services/mapService";

export default {
  name: "AudioVideoCall",
  data() {
    const baseInstructions = `You are a voice assistant for take-a-break, based on GLM model.
...
14. If users ask about topics that may trigger content filters, politely redirect the conversation to appropriate topics related to relaxation, wellness, or general knowledge.`;
    ...
    return {
      ...
      panelParams: {
        ...
        instructions: baseInstructions, // system prompt - fixed, can be extended with location context
        beta_fields: {
  ...
  methods: {
    ...
    async fetchLocationContext() {
      try {
        this.locationStatus = "loading";
        this.locationError = null;
        const position = await getUserPosition();
        this.location = position;
        const spots = await getNearbySpots(position);
        const prompt = buildSystemPrompt(position, spots);
        this.applySystemPrompt(prompt);
        this.locationStatus = "granted";
      } catch (error) {
        console.warn("[AudioVideoCall] Failed to fetch location context", error);
        this.locationStatus = "error";
        this.locationError = "Location unavailable. Using fallback.";
        const prompt = buildSystemPrompt(this.location, await getNearbySpots(this.location));
        this.applySystemPrompt(prompt);
      }
    },
    applySystemPrompt(prompt) {
      this.systemPrompt = prompt;
      const merged = prompt ? `${this.baseInstructions}\n\n${prompt}` : this.baseInstructions;
      this.panelParams.instructions = merged;
      if (this.isConnected) {
        this.sessionUpdate();
      }
    },
