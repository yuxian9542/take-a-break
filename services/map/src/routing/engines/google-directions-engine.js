 ...
 */
export class GoogleDirectionsEngine {
    ...
    async computeRoute(input) {
        try {
            ...
            if (data.status !== 'OK') {
                ...
                throw new RouteEngineError(`Google Directions API error: ${data.status}${data.error_message ? ` - ${data.error_message}` : ''}`);
            }
            ...
            throw new RouteEngineError(error instanceof Error ? error.message : 'Unknown error computing route');
        }
    }
    mapMode(mode) {
        const modeMap = {
...
//# sourceMappingURL=google-directions-engine.js.map