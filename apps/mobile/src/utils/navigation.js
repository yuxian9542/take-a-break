...
export async function startNavigation(destination, options = {}) {
    ...
    try {
        ...
        if (!supported) {
            Alert.alert('Navigation Unavailable', 'Unable to open maps application. Please ensure you have a maps app installed.', [{ text: 'OK' }]);
            return false;
        }
        ...
        Alert.alert('Navigation Error', 'Failed to open navigation. Please try again.', [{ text: 'OK' }]);
        return false;
    }
}
/**
 ...
 */
function buildNavigationUrl(destination, mode, origin) {
 ...
 */
export async function showLocation(destination) {
    ...
    try {
        ...
        const supported = await Linking.canOpenURL(url);
        if (!supported) {
            Alert.alert('Maps Unavailable', 'Unable to open maps application.', [{ text: 'OK' }]);
            return false;
        }
        await Linking.openURL(url);
        return true;
        ...
        Alert.alert('Error', 'Failed to open location. Please try again.', [{ text: 'OK' }]);
        return false;
    }
}
/**
 ...
 */
export async function isNavigationAvailable() {
    try {
