import { mockSpots, RelaxationSpot } from './mockSpots';

export interface BreakStep {
  stepNumber: number;
  type: 'breath' | 'walk' | 'voice' | 'stretch' | 'meditation' | 'activity' | 'wim_hof';
...
export interface BreakPlan {
  ...
  status: 'stressed' | 'tired' | 'need_pause';
  steps: BreakStep[];
}
...
const getAIRecommendedLocations = (
  status: 'stressed' | 'tired' | 'need_pause',
  ...
  let recommendedSpots: RelaxationSpot[] = [];
  ...
  if (status === 'tired') {
    ...
    if (waterfront) recommendedSpots.push(waterfront);
    ...
    if (cafe) recommendedSpots.push(cafe);
    if (quietSpace) recommendedSpots.push(quietSpace);
    if (indoor) recommendedSpots.push(indoor);
  }
...
export const generateBreakPlan = (
  status: BreakPlan['status'],
  duration: number,
